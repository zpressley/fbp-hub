...
**Addendum (8/3):** the first build sourced Draft Picks from `data/draft_order_2026.json`, ported from `trade.js`'s pattern without questioning it. That file is the *already-executed* 2026 keeper draft, built from 2025's final standings (its own `_comment` field says so) — zero forward value for next-year planning, which is what surfaced as "pulling last year's data" once Zach saw it live. The real order rule also isn't simple reverse-standings: the constitution maps final place → pick number through a Championship/Consolation/Elimination bracket structure (Article 4 §06), and 8 of 12 slots depend on playoff games that haven't happened yet — only the bottom-4 Elimination bracket is derivable from current standings alone. Since the tool's actual money math (tax brackets, buy-ins) only depends on which *rounds* a team owns, not pick-within-round order, we fixed this by dropping the stale file dependency entirely: every team now defaults to one projected pick per round (`KEEPER_DRAFT_ROUNDS = 29`, no order/pick-number shown), with a drop toggle to model trading one away and a picker to model acquiring a hypothetical one from another team. `data/standings.json` (live, Yahoo-synced) was confirmed available for a future "projected order" layer if that's ever wanted — not built now, see the options discussed with Zach on 8/3.
